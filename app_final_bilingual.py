...
Final Version: Bilingual AI Responses, No GIFs, Priority Updates
"""
...
# ============================================================================
# BILINGUAL AI RESPONSE FUNCTION
# ============================================================================

def get_ai_response(query, lesson="", context=""):
    """Get AI response from Groq with lesson context - Returns Spanish + English"""
    try:
        api_key = os.getenv("GROQ_API_KEY")
        if not api_key:
            return {
                "spanish": "⚠️ API key not configured.",
                "english": "⚠️ API key not configured."
            }
        
        client = Groq(api_key=api_key)
        lesson_context = f"The user is learning about {lesson}. " if lesson else ""
        
        # FIRST CALL: Get Spanish response
        spanish_response = client.chat.completions.create(
            model="openai/gpt-oss-20b",
            messages=[
                {
                    "role": "system",
                    "content": f"You are a Spanish language tutor helping expats learn Spanish in Querétaro. {lesson_context}{context} Respond ONLY in Spanish. Keep response under 100 words."
                },
                {
                    "role": "user",
                    "content": query
        ...
        spanish_text = spanish_response.choices[0].message.content
        
        # SECOND CALL: Translate to English
        english_response = client.chat.completions.create(
            model="openai/gpt-oss-20b",
            messages=[
                {
                    "role": "system",
                    "content": "You are a translator. Translate the following Spanish text to English. Keep the same meaning, tone, and structure."
                },
                {
                    "role": "user",
        ...
        english_text = english_response.choices[0].message.content
...
# ============================================================================
# TAB 1: CONVERSATIONS (WITH ENGLISH + NO GIFs)
# ============================================================================
...
# ============================================================================
# TAB 3: MARKET (WITH QUICK REF + BILINGUAL AI)
# ============================================================================
        ...
        if has_groq:
            if st.button(f"💡 Get AI help with '{market_query}'", key="market_ai"):
                with st.spinner("Thinking..."):
                    ...
                    if ai_response:
                        st.markdown("### 🇪🇸 Spanish:")
                        st.info(ai_response['spanish'])
                        st.markdown("### 🇬🇧 English:")
                        st.info(ai_response['english'])

# ============================================================================
# TAB 4: HOSPITAL (WITH QUICK REF + BILINGUAL AI)
# ============================================================================
...
# ============================================================================
# TAB 5: PAPELERÍA (WITH QUICK REF + BILINGUAL AI)
# ============================================================================
...
# ============================================================================
# TAB 6: SCHOOL (WITH QUICK REF + BILINGUAL AI)
# ============================================================================
...
✨ Reordered tabs | 📖 Quick References | 📚 15 Verbs | 🔤 English translations | 🇬🇧 Bilingual AI  
